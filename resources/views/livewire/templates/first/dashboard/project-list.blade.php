<div x-data=" { create: false } " class="py-4 px-3">
    ...
    @foreach($projects as $project)
        <div class="relative z-30 w-full h-12 bg-gray-300 cursor-pointer border border-gray-400 my-2 transition duration-300 ease-in-out transform hover:scale-95 hover:shadow flex flex-row justify-between items-center px-4">
            <p onclick="window.location.href = '{{ route('dashboard.edit', ['id' => $project->id]) }}'">{{$project->title}}</p>
            <div class="transition duration-300 ease-in-out group z-40">
                ...
                <ul class="absolute bg-white border border-black rounded-sm right-4 hidden group-hover:block">
                    <li onclick="window.location.href = '{{ route('signup', ['id' => $project->id]) }}'" class="hover:bg-slate-200 px-2 py-0.5">show form</li>
                    <li onclick="window.location.href = '{{ route('project.detail', ['id' => $project->id]) }}'" class="hover:bg-slate-200 px-2 py-0.5">Details</li>
                    <li onclick="window.location.href = '{{ route('dashboard.delete', ['id' => $project->id]) }}'" class="hover:bg-slate-200 px-2 py-0.5">Delete</li>
                </ul>
            </div>
        </div>
